...
class VisualES():
    ...
    def update_or_create_new_track(self, track_id, bbox, idx_frame, all_tracks, mark_old_track_idx, all_es_feat_tracks, all_start_end_offset_track, all_emotion_category_tracks, es_feature, emotion_cat):
        ...
        similar_track_ids = []
        for each_active_tracks in all_tracks:
            if each_active_tracks['id'] in mark_old_track_idx:
                continue
            ...
            if iou > self.args.network.iou_threshold:
                similar_track_ids.append(each_active_tracks['id'])
        ...
        if len(similar_track_ids) > 0:
            ...
            if best_match_track_id is not None:
                best_match_track_id -= 1 # to get position in list
                ...
                # Concatenate the new ES feature to the existing ES features for this track
                new_es_array_track = np.array([es_feature])
                all_es_feat_tracks[best_match_track_id] = np.concatenate([all_es_feat_tracks[best_match_track_id], new_es_array_track])

                # Update the end frame of this track in the start-end-offset list
                all_start_end_offset_track[best_match_track_id][-1] = idx_frame

                # Update the emotion category for this track
                all_emotion_category_tracks[best_match_track_id] = np.concatenate([all_emotion_category_tracks[best_match_track_id], [emotion_cat]])
    ...
    def x1x2y1y2_to_xywh(self, bbox):
        x1, y1, x2, y2 = bbox
        x = x1
        y = y1
        w = x2 - x1
...
if __name__ == "__main__":
    ...
    for i in all_tracks_filter:        
        print(f"Emotion list for track {i}: ", all_emotion_category_tracks[i-1])
        print(f"Emotion features shape for track {i}: ", all_es_feat_tracks[i-1].shape)
        print("\n")
